...
with tab3:
    ...
    with col_vis3:
        ...
        with tab_basic:
            ...
            ##### 📊 5개년 자산 로드맵 
            <span title="상세 계산 로직: 
            1. 매월 말일 투자액 기준 연 복리 적용 (연 수익률/12개월) 
            2. 원금 = 월 투자액 × 누적 개월 수 
            3. 손익 = 미래가치(FV) - 누적 원금 
            4. 월별로 발생하는 이자가 다음 달 원금에 가산되는 복리식 산출">ℹ️ (마우스를 올리면 상세 로직 확인)</span>
        ...
        with tab_bonus:
            ...
            <span title="상세 계산 로직: 
            1. 기본급 산정: (연봉 ÷ 20) 
            2. PI(생산성 격려금): 기본급의 200% (연간) 
            3. PS(초과이익 성과급): 기본급의 1,000% 가정 
            4. 세후 처리: 위 합산액에서 추정 세율 38% 공제 적용 
            5. 모든 성과급은 연차별로 누적 합산하여 복리 계산">ℹ️ (마우스를 올리면 상세 로직 확인)</span>
            """, unsafe_allow_html=True)
