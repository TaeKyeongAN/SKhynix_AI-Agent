...
st.title(f"{mode}")
col_visual, col_chat = st.columns([6, 4]) # 6:4 비율 화면 분할 유지
...
with col_chat:
    ...
    if mode == "⏱️ 갓생(God-생) 루틴 메이커":
        ...
        """
        greeting = f"안녕하세요 안태경 님! 오늘 컨디션은 '{condition}' 상태시군요. 좌측 데이터를 보며 스크롤을 내려도 저는 항상 여기 있으니, 언제든 분석을 요청해 주세요!"
    else:
        ...
        greeting = f"재무 상담가입니다. 현재 저축률을 {save_ratio}%로 설정하셨네요! 좌측 소비 내역을 보시면서 궁금한 점이 있다면 바로바로 말씀해 주세요."

    if "current_mode" not in st.session_state or st.session_state.current_mode != mode:
        st.session_state.messages = [{"role": "assistant", "content": greeting}]
        st.session_state.current_mode = mode

    # 채팅 내역이 길어질 경우를 대비한 컨테이너
